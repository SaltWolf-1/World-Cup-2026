...
import {
  ...
  Radio, Star, ChevronRight, Ticket, UserCircle, LogIn,
} from "lucide-react";
...
const RED_GLOW_SM  = "shadow-[0_0_10px_rgba(232,25,60,0.3)]";
const RED_GLOW_MD  = "shadow-[0_0_14px_rgba(232,25,60,0.25)]";
const RED_GLOW_LG  = "shadow-[0_0_20px_rgba(232,25,60,0.5)]";
  ...
  return (
    <Link
      ...
      className={cn(
        ...
        isActive
          ? `bg-primary text-primary-foreground ${RED_GLOW_MD}`
...
function DrawerAuthRow({ onClose }: { onClose?: () => void }) {
  const { user, isAuthenticated, login, logout } = useAuth();
  if (isAuthenticated && user) {
    const displayName = [user.firstName, user.lastName].filter(Boolean).join(" ") || user.email || "Fan";
    return (
      <div className="px-4 pb-5 pt-3 border-t border-border flex items-center gap-3">
        {user.profileImageUrl ? (
          <img src={user.profileImageUrl} alt={displayName} className="w-8 h-8 rounded-full object-cover border border-primary/30" />
        ) : (
          <div className="w-8 h-8 rounded-full bg-primary/10 border border-primary/30 flex items-center justify-center">
            <span className="text-xs font-black text-primary">{displayName[0]?.toUpperCase()}</span>
          </div>
        )}
        <Link href="/profile" onClick={onClose} className="flex-1 min-w-0">
          <p className="text-sm font-bold truncate">{displayName}</p>
          <p className="text-[10px] font-mono text-muted-foreground uppercase tracking-wider">View profile</p>
        </Link>
        <button onClick={logout} className="text-[10px] font-mono uppercase text-muted-foreground hover:text-destructive transition-colors">
          Out
        </button>
      </div>
    );
  ...
  return (
    <div className="px-4 pb-5 pt-3 border-t border-border flex items-center justify-between">
      ...
      <button
        onClick={login}
        className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-primary text-primary-foreground text-xs font-bold font-mono uppercase tracking-wider hover:brightness-110 transition-all"
      >
    ...
    return (
      <div className="p-4 border-t border-border flex items-center gap-3">
        <Link href="/profile" className="flex items-center gap-2.5 flex-1 min-w-0 group">
          {user.profileImageUrl ? (
            <img src={user.profileImageUrl} alt={displayName} className="w-7 h-7 rounded-full object-cover border border-primary/30 group-hover:border-primary/60 transition-colors" />
          ) : (
          ...
          <div className="min-w-0">
            <p className="text-xs font-bold truncate group-hover:text-primary transition-colors">{displayName}</p>
          </div>
        </Link>
        <button onClick={logout} className="text-[10px] font-mono uppercase text-muted-foreground hover:text-destructive transition-colors shrink-0">Sign out</button>
      </div>
    );
  }
  return (
    <div className="p-4 border-t border-border flex items-center justify-between">
      <div className="flex items-center gap-2">
        <Radio className="w-3 h-3 text-primary animate-pulse" />
        <span className="text-[10px] text-muted-foreground font-mono uppercase tracking-wider">Live Feed</span>
      </div>
      <button
        onClick={login}
        className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-primary text-primary-foreground text-xs font-bold font-mono uppercase tracking-wider hover:brightness-110 transition-all"
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-border">
          <Link href="/" onClick={onClose} className="flex items-center gap-3 group">
            <div className={cn(
              "w-8 h-8 bg-primary text-primary-foreground flex items-center justify-center rounded-sm font-black text-lg italic skew-x-[-10deg]",
              RED_GLOW_SM,
            )}>
              W
            </div>
            <div className="flex flex-col leading-none">
              ...
              <span className="text-[9px] font-mono text-muted-foreground uppercase tracking-widest">USA · Canada · Mexico</span>
            </div>
          </Link>
          <SheetClose asChild>
            <button
  ...
  return (
    <header className="md:hidden h-14 border-b border-border bg-background flex items-center justify-between px-4 z-10 shrink-0">
      <Link href="/" className="flex items-center gap-2">
        <div className={cn(
          "w-7 h-7 bg-primary text-primary-foreground flex items-center justify-center rounded-sm font-black text-base italic skew-x-[-10deg]",
          RED_GLOW_SM,
        )}>
          W
        </div>
        <div className="flex flex-col leading-none">
          ...
          <span className="text-[8px] font-mono text-muted-foreground/60 uppercase tracking-wider hidden xs:block">
            USA · Canada · Mexico
          </span>
        </div>
      </Link>
            ...
            >
              <item.icon
                className={cn(
                  ...
                  isActive && "drop-shadow-[0_0_6px_rgba(232,25,60,0.8)]",
                )}
              />
              <span
                className={cn(
...
export function Layout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex h-dvh w-full bg-background overflow-hidden text-foreground">
      ...
      <aside className="w-64 flex-shrink-0 border-r border-border bg-sidebar hidden md:flex flex-col">
        <div className="p-5 border-b border-border">
          <Link href="/" className="flex items-center gap-3 group">
            <div className={cn(
              "w-9 h-9 bg-primary text-primary-foreground flex items-center justify-center rounded-sm font-black text-xl italic skew-x-[-10deg] group-hover:scale-110 transition-transform",
              RED_GLOW_LG,
            )}>
              W
            </div>
      ...
      {/* Mobile drawer (from header hamburger) */}
      ...
      <main className="flex-1 flex flex-col overflow-hidden relative min-h-0">
        ...
        <div className="flex-1 overflow-y-auto overscroll-contain p-4 pb-24 md:pb-8 md:p-8">
